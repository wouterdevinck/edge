...
namespace iotedgeapiclient.unixsocket {
    ...
    internal class HttpRequestResponseSerializer {
        ...
        public async Task<HttpResponseMessage> DeserializeResponse(HttpBufferedStream bufferedStream,
        ...
        async Task SetHeadersAndContent(HttpResponseMessage httpResponse, HttpBufferedStream bufferedStream,
            CancellationToken cancellationToken) {
            ...
            foreach (KeyValuePair<string, string> contentHeader in contentHeaders) {
                ...
                if (string.Equals(contentHeader.Key, ContentLengthHeaderName,
                    StringComparison.InvariantCultureIgnoreCase)) {
                    if (!long.TryParse(contentHeader.Value, out long contentLength)) {
                        throw new HttpRequestException(
                            $"Header value {contentHeader.Value} is invalid for {ContentLengthHeaderName}.");
                    }
        ...
        async Task SetResponseStatusLine(HttpResponseMessage httpResponse, HttpBufferedStream bufferedStream,
            CancellationToken cancellationToken) {
